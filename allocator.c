...
struct memory_blob * alloc_new_blob(size_t size)
{
    ...
    if (blob == NULL) {
        cclynx_fatal_error("FATAL ERROR: failed to allocate memory blob\n");
    }
    memset(blob, 0, sizeof(struct memory_blob));
    blob->memory = calloc(1, size);
    if (blob->memory == NULL) {
        cclynx_fatal_error("FATAL ERROR: failed to allocate memory blob data (%zu bytes)\n", size);
    }
    blob->capacity = size;
    return blob;
}
...
struct memory_blob_pool * memory_blob_pool_create(size_t blob_size, size_t alignment)
{
    ...
    if (pool == NULL) {
        cclynx_fatal_error("FATAL ERROR: failed to allocate memory blob pool\n");
    }
    ...
    if (pool->blobs == NULL) {
        cclynx_fatal_error("FATAL ERROR: failed to allocate memory blob pool entries\n");
    }
    return pool;
}
...
void * memory_blob_pool_alloc(struct memory_blob_pool * pool, size_t size)
{
    ...
    if (pool->blob_count >= pool->blob_capacity) {
        ...
        if (new_blobs == NULL) {
            cclynx_fatal_error("FATAL ERROR: failed to reallocate memory blob pool entries\n");
        }
        pool->blobs = new_blobs;
    }

    if (aligned_size > pool->blob_size) {
        cclynx_fatal_error("FATAL ERROR: allocation of %zu bytes exceeds blob size of %zu bytes\n", size, pool->blob_size);
    }
